...
public class Davinci : MonoBehaviour
{
    ...
    private RendererType rendererType = RendererType.uiImage;
    ...
    private UnityAction<Sprite> onReadyImageAction;
    ...
    public static void addToDB(string name, int size)
    {
        AssetPublicParent assetPublic = Constants.getAssetPublicParent();
        AssetPublic asset = new AssetPublic();
        asset.name = name;
        asset.size = size;
        assetPublic.list.Add(asset);
        Constants.setAssetPublicParent(assetPublic);
        assetPublic = null;
    }

    public static void removeDB()
    {
        AssetPublicParent assetPublic = new AssetPublicParent();
        Constants.setAssetPublicParent(assetPublic);
        assetPublic = null;
    }

    public static bool checkAssetOriginal(string name, int size)
    {
        AssetPublicParent parent = Constants.getAssetPublicParent();
        bool exist = false;
        foreach(AssetPublic asset in parent.list)
        {
            if(asset.name == name && asset.size == size)
            {
                exist = true;
            }
        }
        //if(!exist)
            //Debug.Log("file no exist " + name + " " + size);
        parent = null;
        return exist;
    }
    ...
    public Davinci withImageReadyAction(UnityAction<Sprite> action)
    {
        this.onReadyImageAction = action;
    ...
    private IEnumerator Downloader()
    {
        ...
        if (www.error == null)
        {
            ...
            addToDB(uniqueHash, www.bytes.Length);
        }
    ...
    private IEnumerator ImageLoader(Texture2D texture = null)
    {
        ...
        if (texture == null)
        {
            ...
            if (!checkAssetOriginal(uniqueHash, fileData.Length))
            {
                fileData = null;
                success = false;
